...
impl<Front: SocketHandler, L: ListenerHandler + HttpListenerHandler> Http<Front, L> {
    ...
    pub fn print_state(&self, prefix: &str) -> String {
      ...
      prefix, self.request_state, self.request_header_end, self.response_state, self.response_header_end)
    }
    ...
    /*
    pub fn frontend_socket_mut(&mut self) -> &mut TcpStream {
        self.frontend_socket.socket_mut()
    }

    pub fn backend_socket(&self) -> Option<&TcpStream> {
        self.backend_socket.as_ref()
    }

    pub fn backend_socket_mut(&mut self) -> Option<&mut TcpStream> {
    ...
    */
    ...
    pub fn frontend_hup(&mut self) -> SessionResult {
        SessionResult::CloseSession
    }
    */

    pub fn backend_hup(&mut self) -> SessionResult {
                ...
                if self.request_state == Some(RequestState::Initial) {
                    ...
                    self.log_request_error(
                        ...
                        &format!(
                            ...
                            frontend_readiness, backend_readiness, size
                        )
                    );
                }
                return SessionResult::CloseSession;
...
impl<Front: SocketHandler, L: ListenerHandler + HttpListenerHandler> Http<Front, L> {
    ...
    fn ready_inner(
        ...
        while counter < max_loop_iterations {
            ...
            if backend_interest.is_error() && self.backend_hup() == SessionResult::CloseSession {
                self.frontend_readiness.interest = Ready::empty();
                self.backend_readiness.interest = Ready::empty();
            ...
            match (peer_ip, peer_port, frontend_ip) {
                (IpAddr::V4(_), peer_port, IpAddr::V4(_)) => {
                    if let Err(e) = write!(
                        &mut s,
                        "proto={};for={}:{};by={}\r\n",
                        proto, peer_ip, peer_port, frontend_ip
                    ) {
                        error!("could not append request header: {:?}", e);
                    }
                }
